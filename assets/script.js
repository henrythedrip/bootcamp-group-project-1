...
// this demenstrates how to add a marker to the map using advanced markers API
...
// created markers for each user. 
  let markerArray = [];
  userData.forEach((user) => {
    const marker = new AdvancedMarkerElement({
      map: map,
      position: user.mapOptions.position,
      title: user.name,
    });
    markerArray.push(marker);
  });

  // added event listner for each marker.
  // !TODO!: create infowindow when clicked. 
  markerArray.forEach((marker) => {
    marker.addListener("click", (e) => {
      console.log("clicked");
    });
  
    // creating contentstring forInfoWindows
    const contentString = mapInfoWindow + mapOptions; ypt
    ewq                  
...
//     markerArray[i].addListener("click", (event) => {
//       console.log("clicked");
//     });
...
//   //     var contentString = userData[i].mapOptions;

//   //     const userData = google.maps.importLibrary("maps")({
//   //       position: userData[0].mapOptions,
//   //       map: map,
//   //     });
